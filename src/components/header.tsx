...
export default function Header({ links }: HeaderProps) {
  ...
  // The home page renders its own sticky sidebar that handles navigation,
  // so suppress the floating top navbar there to keep the layout clean.
    ...
    <header className="hidden md:flex items-center justify-center fixed z-[999] w-full mt-3">
      ...
      >
        <ul className="flex items-center gap-0.5 text-[0.8rem] font-medium text-gray-500">
          {links.map((link) => {
            const isExternal = /^https?:\/\//.test(link.hash);
